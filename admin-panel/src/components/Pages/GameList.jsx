...
const GamesList = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        // Fetch games and categories in parallel
        ...
        if (gamesResponse.data && gamesResponse.data.data && Array.isArray(gamesResponse.data.data)) {
          ...
          console.warn('Games data is not an array:', gamesResponse.data);
          setGames([]);
        }
  ...
  const filteredGames = React.useMemo(() => {
    // Make sure games is an array before filtering
    if (!Array.isArray(games)) {
      console.warn('Games is not an array in filteredGames calculation');
      return [];
    }
